 ...
 * }
*/
function makePersonObject(paraId, paraName, paraEmail) {
  let person = {
  id: paraId,
  name:  paraName,
  email: paraEmail
...
*/
function getName(paraObject) {
  return (`Hello, my name is ${paraObject.name}`)
}
...
*/
function makeSmartPerson(paraName) {
  let smartPerson = {
    name : paraName, 
    sum : (a,b) => a+b,
...
function getCarInfoByIndex(inventory, index) {
  return `this car is the ${inventory[index].car_make} ${inventory[index].car_model}`
}
 ...
 * it will return `This is a Lincoln Navigator`.
*/
function getCarInfoById(carArray, idNum) {
  if(carArray[idNum - 1 ].id === idNum){
    return `This is a ${carArray[idNum-1].car_make} ${carArray[idNum-1].car_model}`
    }
  }
/**
...
function getModelYears(carArray) {
  ...
  for(let i=0; i<carArray.length; i++){
    carYearsArray.push(carArray[i].car_year)
  }
...
function getGermanCars(carsArray) {
  ...
  for(let i=0; i < carsArray.length; i++){
    if(carsArray[i].car_make === "Audi" || carsArray[i].car_make === "BMW" || carsArray[i].car_make === "Mercedes-Benz" || carsArray[i].car_make === "Volkswagen" ){
